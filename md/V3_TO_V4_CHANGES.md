...
타임라인이 그대로 보여준다 — 로컬 memcached 라면 해시 조회(③)에서 값이
나와 바로 응답(⑮)으로 갔다. **④~⑫(GET)·②~⑩(SET)이 원격이라서 통째로
추가된 일**이다.

| | 잰다 | 기준에 비추면 |
|---|---|---|
| **span v2** | 장치가 일한 시간 (GET ⑨~⑫ / SET ④~⑧) | 틀리진 않다 — 추가된 일 중 **장치 활동만** 본다 |
| **span v3** | 추가된 일 전체 (GET ④~⑫ / SET ②~⑩) | **기준과 일치.** `admit`·`ret` 도 로컬이면 없는 대기다 |
...
> ⑬~⑮(완료 주차 → 재개 → 송신)는 두 정의 다 밖이다. 기준상 원격이 만든
> 구간이지만 v3 의 끝을 "상태 동등 지점" 으로 놓아 밖에 남고, 별도 계측
> (`post`)으로 추적한다 — §5-3.

### 1-4. 왜 `ret` 은 SET 에만 있나

타임라인이 부르는 정당한 질문: GET 은 주차·재개(⑬⑭)가 밖인데 SET 은 ⑨가
`ret` 으로 안이다 — 통일해야 하지 않나?
