...
>
> **정의의 기준** — span 의 목적이 **비밀 실행 환경용 원격 메모리 접근
> 지연**이므로, **"로컬 메모리였으면 하지 않았을 일"을 전부 포함**한다.
> v2(전송+암복호)는 그중 장치 활동만 재는 부분 정의다.
>
> **요청 단계 위에 두 구간을 그은 그림**이
> [`EXTENDED_SPAN_DIAGNOSIS.md`](EXTENDED_SPAN_DIAGNOSIS.md) §1 에 있다.
> 정의를 처음 보는 사람은 그 그림부터 볼 것 — 아래 경계표만으로는 안 보인다.
>
> ```text
> GET   v2 는 RDMA post 부터, v3 는 backend 진입부터. 끝(복호)은 같다.
>       차이 = admit                  제출을 기다린 시간
>
> SET   v2 는 seal→CQE, v3 는 진입→WFLIGHT 해제. 앞뒤가 다 넓어졌다.
>       차이 = admit (앞) + ret (뒤)
> ```
>
> **두 정의 다 "완료 주차 → 재개 → 응답 송신"(`post`)은 안 잰다.**
> 운영점에서 그것이 span v3 의 16 배다 —
