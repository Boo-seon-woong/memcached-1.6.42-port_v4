...
while IFS=$'\t' read -r kind lbl a b c d rest; do
  ...
  if [ "$kind" = client ]; then
    go_dist "$lbl" "$a" "$b" "$c" "$d" \
      "port_v4 b7fe2984 운영점 서버 고정 — 보충 재측정 (mtT=$a c=$b pipe=$c d=$d)"
  else
    ow=$b; [ "$b" = 0 ] && ow=16; WIRE=$((a*ow)); SS=${rest:-$((WIRE*2>64?WIRE*2:64))}
    SLOT=256 INLINE=1 AD=0 SS=$SS RE=8 PC=$d SQ=1 DEM=0 DVAL=64 NQP=$a ORD=$b MCT=$c \
      CPUSET="0-$((c-1))" arm "$lbl" || { say "무장 실패 $lbl"; continue; }
    go_dist "$lbl" "$c" 4 256 64 "port_v4 b7fe2984 nqp=$a ORD=$b mcT=$c chain=$d — 보충 재측정"
  fi
  sfsave
done < "$LIST"
say "보충 종료"
