...
| 대기 | v3 | 원인 | v4 의 처리 |
|---|---:|---|---|
| 제출 | 9.91 µs | 읽기 버퍼를 다 파싱해야 제출 | 파싱 즉시 post (`ext_submit_inline`) |
| 완료 관측 | 31.56 µs | CQE 가 pass 끝까지 **폴링조차 안 됨** | post 자리에서 수거 (`ext_reap_every`) |
| SET 재개 | 25.48 µs | 거두고도 재개를 다음 pass 로 | 거둔 그 pass 에서 재개 |

**SET 이 답을 갖고 있었다.** v3 의 pac(publish-at-command)은 admit 이 처음부터
0.5~0.7 µs 였다 — 큐에 넣자마자 그 자리에서 flush 했기 때문이다. **GET 만
상류의 배칭을 물려받고 있었고**, v4 는 GET 을 SET 처럼 만든 것이다.
...
post 조차 안 된다.** 그것이 제출 대기 9.91 µs 다.
...
storage_store_item_pac
 ...
 ⇣
 (WRITE CQE)
 └ 거둔다 → **재개는 다음 pass 로 미룬다**   ← 25.48 µs
```
