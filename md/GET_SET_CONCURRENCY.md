...
> 기준이며, 위 수치는 기준선 기록이다.~~
>
> **폐기 (2026-07-30).** 계약이 바뀌었다 — 이제 GET-only와 1:10 혼합 **양쪽**
> 10 M이고 **SET span도 30 µs 미만**이어야 한다. SET은 게이트 대상이다.
...
## 6. 종결 (2026-07-31) — §4의 미해결이 규명됐다

§4가 남긴 질문은 "SET 1건당 워커 점유가 span의 16배인데 `busyCPU`는
23.2/30으로 **포화도 아니다** — 남은 시간은 어디 있나"였다. 두 조각으로
나뉘어 각각 답이 나왔다.

**조각 1 — 점유가 span보다 긴 것: pac이 없앴다.** 워커가 자기 WRITE CQE를
busy-wait하던 구조였고, 스텁을 커맨드 시점에 게시하고 `STORED` 응답만 CQE로
미루자 워커가 풀렸다. SET-only 0.311 M → 2.348 M.

**조각 2 — 포화가 아닌 것: 전역 `e->mutex`였다.** pac 이후에도 SET-only는
`busyCPU 23.3/30`으로 4.7코어가 놀았다. perf가 **SET CPU의 13.21%를
`native_queued_spin_lock_slowpath`**로 잡았고, 호출자는
`extstore_alloc`의 futex wake → `try_to_wake_up` → 런큐 스핀락이었다.
워커 전용 loc magazine이 **LIFO 최상단의 `len` 일치**만 보는 바람에
키 길이가 7종인 워크로드에서 6/7이 전역 락으로 떨어지고 있었다.

배열을 스캔하도록 고치자 **busyCPU 23.3 → 28.0**으로 포화가 회복되고
SET-only가 2.640 M → 4.121 M(+56%)이 됐다. 이제 SET-only의 실효 CPU/op와
혼합에서 푼 `C_set`이 처음으로 일치한다(6.79 vs 6.63 µs).

### 그리고 GET/SET 비대칭에 대한 결론이 바뀌었다

이 문서 전체가 "SET이 op당 훨씬 비싸다"를 전제로 SET을 봤다. 사실이다 —
`C_set 6.63 µs` 대 `C_get 2.369 µs`로 2.8배다. 그러나 **1:10 혼합에서 GET이
CPU의 78.1%를 쓴다.** 비중을 곱하면 같은 1% 절감의 가치가
**GET : SET = 3.6 : 1**이고, 실제로 마지막 3.4%를 넘긴 것은 GET 쪽 절감이었다
(상승분의 105%). 비대칭을 op당 비용으로만 읽으면 레버를 잘못 잡는다.

상세는 [`SET_CAMPAIGN_HANDOFF.md`](SET_CAMPAIGN_HANDOFF.md) §14~§16.
