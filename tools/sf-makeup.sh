...
# ═══════════════ 보충 측정 ═══════════════
# 유실 사유: genie 사용한도 초과로 90분 무보고 → 30분 창 3회 소진(C2·C3·C4),
#            그리고 D4 는 GO 도착 시 bed 가 이미 d=8 이라 genie 가 정당 거부.
# 어느 쪽도 "틀린 값이 보고된" 오염이 아니라 통째 유실이다 (부분 셀 0개 확인).
say "보충 구동기 대기 — 본 구동기 종료까지"
while pgrep -f 'bash /tmp/sfr.sh' >/dev/null; do sleep 120; done
say "본 구동기 종료 확인 — 보충 시작"

for c in 2 3 4; do
  SLOT=256 INLINE=1 AD=0 WW=64 RE=8 PC=$c SQ=1 DEM=0 DVAL=64 arm "SF3-C$c" || continue
  go_trio "SF3-C$c" 30 4 256 64 "port_v4 c11ede3e slot=256 W=64 ext_post_chain=$c (reap=8) — 보충 측정"
done
sfsave

if flush_preload 4; then
  go_trio "SF3-D4" 30 4 256 4 "port_v4 c11ede3e slot=256 W=64 (재기동 없음 — flush 후 d=4 재프리로드) — 보충 측정" \
