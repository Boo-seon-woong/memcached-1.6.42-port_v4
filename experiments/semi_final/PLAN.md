...
3  SF-E{1x1024,2x512,8x128,16x64,32x32,64x16,128x8}  7×3   클라만 변경(-c·pipe). 재기동 없음
4  value 축   탐침: flush → d=156 프리로드 → fallback 확인 → CAP 확정
   SF-D{16,32,128,CAP}      4×3        크기마다 flush+프리로드. 부하 -d 동일
5  SF-C{1..7,9..16}         15×3       재기동 15회: PC=c RE=8 → 20 1280 4 1280, 매회 d=64 프리로드+게이트
6  SF-Q{1,2,8,16,64}        5×3        재기동 5회: → 20 1280 q 1280 (ORD=0 협상)
7  SF-O{1,2,4,8,32,64}      6×3        재기동 6회: ORD=o 핀 → 20 1280 4 1280
8  SF-S{1x256,2x128,8x32,32x8,64x4}  5×3   재기동 5회: ORD=o 핀 → 20 1280 q 1280
9  SF-T{1,2,4,8,12,16,24,28}     8×3   재기동 8회: MCT=m CPUSET=0-(m−1) → 20 1280 4 1280, genie -t m
10 SF-OP-r2                 3부하       OP 재무장(재기동 1회) 후 1과 동일 조건
```
...
```text
부하    (1+7+7+4+15+5+6+5+8+1) = 59 구성 × 3 = 177 부하 × 80초  ≈ 236분
재기동  40회 (+프리로드)                                       ≈ 100분
