...
void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("ext_drain_empty_max", "%u", settings.ext_drain_empty_max);
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_new_cmd:
            ...
            --nreqs;
            /* backend 체류 상한. 링크 왕복이 177.8 µs 라 10 M 을 내려면 in-flight
             * 1,778 건이 물리적으로 필요하다 — 문제는 몇 건이 떠 있느냐가 아니라
             * 어디서 기다리느냐다. 여기서 멈추면 초과분은 이미 읽어둔 소켓
             * 버퍼에 남는다(아래 rbytes>0 경로가 그대로 보존한다). 서버 메모리가
             * 유계가 되고 span v3 는 모집단이 줄어 서비스 시간에 수렴한다. */
            if (nreqs >= 0 &&
                (settings.ext_admit_max == 0 ||
                 c->thread->ext_resident < settings.ext_admit_max)) {
                reset_cmd_handler(c);
            } else if (c->resp_head) {
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
       ...
       case 'N':
        ...
        case 'o': /* It's sub-opts time! */
            ...
            case ITEM_MAG_DEPTH:
                if (subopts_value == NULL) {
                    fprintf(stderr, "Missing numeric argument for item_mag_depth\n");
                    goto error;
                }
