...
관측하는 문맥은 그 락을 이미 쥐고 있을 수 있다(큐 상한 flush·meta-get —
§3-2). 그래서 실현이 락-안전 방출 지점 뒤로 밀리고, 그 대기가 `ret` 이다 —
정의가 SET 을 다르게 취급해서가 아니라 **read/write 의 본성 차이**다.
실측 0.10~0.62 µs 로 작다. EXP-0 의 173~2372 µs 는 이 대기가 아니라
방출구가 `if (out)` 에 막혀 있던 것이다(§2-4).
