...
static void process_lru_command(conn *c, mcp_parser_t *pr) {
    /* v2: no LRU exists (remote-only stubs). */
    out_string(c, "CLIENT_ERROR unsupported in v2 (no LRU)");
}
...
static void process_slabs_command(conn *c, mcp_parser_t *pr) {
    /* v2: single-purpose stub classes; there is no slab mover. */
    out_string(c, "CLIENT_ERROR unsupported in v2 (no slab mover)");
}

static void process_lru_crawler_command(conn *c, mcp_parser_t *pr) {
    /* v2: crawler removed; nothing to crawl without an LRU. */
    out_string(c, "CLIENT_ERROR unsupported in v2 (no LRU crawler)");
}
#ifdef TLS
