...
| 9 | 키 분포 | **uniform**, zipfian | 2 | 클라 `--key-pattern` 만 (서버 불변) |
...
부하  70 × 3 = 210 × (60초 + 사이 20초)                    ≈ 258분
재기동 41회 (+프리로드) · value flush 9회                    ≈ 110분
합계                                                        ≈ 6 시간
```

## 5. 9번 축 — 키 분포 (uniform vs zipfian)

분포만 바꾸고 **나머지는 전부 고정 조건**을 따른다. 서버는 건드리지 않으므로
재기동도 재프리로드도 없다 — P·E 축과 같은 클라 전용 축이다.

```text
R5-U   --key-pattern=R:R                        uniform (고정 조건 그대로)
R5-Z   --key-pattern=Z:Z --key-zipf-exp=0.99    zipfian (YCSB 기본 지수)
각각 YCSB C → B → A, 60초
```

`R5-U` 는 `R5-OP` 와 같은 구성이지만 따로 잰다 — 같은 bed 상태에서 연달아
찍어야 분포 효과를 짝으로 읽을 수 있다(재기동·드리프트가 끼지 않는다).
