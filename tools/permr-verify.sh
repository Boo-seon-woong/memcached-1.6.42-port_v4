...
# ═══════════════ 워커별 MR 검증 (무장 전용) ═══════════════
# 앞선 탐침(prb.sh)은 단일 MR 빌드에서 wire 320·384·512 가 전부 실패해야 한다.
# 여기서는 같은 값들 + Q64 를 워커별 MR 빌드로 무장한다 — 전부 성공해야 한다.
export BIN='$HOME/coherent-mr-v2/bin/memcached.permr'
say "워커별 MR 검증 대기"
while pgrep -f 'bash /tmp/r4.sh' >/dev/null || pgrep -f 'bash /tmp/r4s.sh' >/dev/null \
   || pgrep -f 'bash /tmp/prb.sh' >/dev/null; do sleep 120; done
say "워커별 MR 검증 시작"
for spec in 20:16 24:16 32:16 64:16; do
  q=${spec%%:*}; o=${spec##*:}
  WIRE=$((q*o)); SS=$((WIRE*2)); PW=$(python3 -c "print(f'{$SS*256/1048576:.3f}')")
  TOT=$(python3 -c "print(f'{30*$SS*256/1048576:.2f}')")
  if SLOT=256 INLINE=1 AD=0 SS=$SS RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=$q ORD=0 \
       MCT=30 CPUSET=0-29 arm "PERMR-nqp$q" >/dev/null 2>&1; then
    say "nqp=$q wire=$WIRE slots=$SS 워커당 ${PW}MB 총 ${TOT}MB → 무장 성공"
  else
    say "nqp=$q wire=$WIRE slots=$SS 워커당 ${PW}MB 총 ${TOT}MB → 무장 실패"
  fi
done
say "검증 종료 — 운영점 복귀"
SLOT=256 INLINE=1 AD=0 SS=128 RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=4 ORD=0 MCT=30 CPUSET=0-29 \
  arm PERMR-restore-OP >/dev/null 2>&1 && say "운영점 복귀 완료" || say "운영점 복귀 실패"
