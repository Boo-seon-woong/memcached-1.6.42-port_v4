...
앵커: 진입(`storage_store_item_pac()` 첫 줄), seal(`pac_seal`), CQE 관측,
`ITEM_WFLIGHT` 해제. SET 의 제출 경로(①~⑥)는 두 버전이 같고(pac),
**⑦ 하나가 다르다.**
...
v3 — 조건부 재개                                ret 173~2,372 µs
 ...
 ⑦ ── if(out) 이 참인 pass 가 올 때까지 ──      ← §2-4 의 결함
...
같은 것이 아니다 — 한 그림에 겹치면 v4 에도 대기 단계가 있는 것처럼 보인다.

### 1-3. 정의의 기준, 차이, 증거

span latency 의 목적은 **비밀 실행 환경에서의 원격 메모리 접근 지연**이다.
그 목적에서 기준이 나온다:

> **로컬 메모리였으면 하지 않았을 일을 전부 포함한다.**

타임라인이 그대로 보여준다 — 로컬 memcached 라면 해시 조회에서 값이 나와
바로 응답으로 갔다. **backend 진입부터 복호(GET)/해제(SET)까지가 원격이라서
통째로 추가된 일**이다.

| | 잰다 | 기준에 비추면 |
|---|---|---|
| **span v2** | 장치가 일한 시간 (GET post→복호 / SET seal→CQE) | 틀리진 않다 — 추가된 일 중 **장치 활동만** 본다 |
| **span v3** | 추가된 일 전체 (GET 진입→복호 / SET 진입→해제) | **기준과 일치.** `admit`·`ret` 도 로컬이면 없는 대기다 |

경계도 기준에서 나온다:

```text
v3 시작   backend 진입 — 로컬과 갈라지는 분기점 (로컬이면 여기서 memcpy 로 끝)
v3 끝     로컬이었을 때와 같은 상태가 되는 지점
          GET: 복호 완료    (값이 로컬 메모리에 실재)
          SET: WFLIGHT 해제 (아이템이 로컬 아이템처럼 행동)
```

GET 은 시작만 당겨졌고 SET 은 앞뒤가 다 넓어졌다 — §2 의 측정에서 SET 이
훨씬 크게 튀는 이유다.

세 성분(`admit`·`v2`·`ret`)은 **각각 독립 집계**라 `admit + v2 + ret = v3`
가 정합성 검사로 성립한다(EXP-0 GET-only `217.12+25.16=242.28` vs 실측
`242.29`). stats 는 `_e2e_avg_ns`(v3) / `_avg_ns`(v2) 로 갈리고,
`extstore_prof_span_ver` 는 `3` 이어야 한다.

이 기준이 탁상 정의가 아니라는 증거는 확장(2026-08-01) 전에 이미 있었다:
v2 로 계약을 통과하던 그 시각(GET·SET 7.8 µs), memtier 는 SET 지연을
**7.45 ms** 로 보고하고 있었다 — 955 배가 전부 계측 밖 대기였다.

> 재개 → 송신은 두 정의 다 밖이다. 기준상 원격이 만든
> 구간이지만 v3 의 끝을 "상태 동등 지점" 으로 놓아 밖에 남고, 별도 계측
> (`post`)으로 추적한다 — §5-3.

### 1-4. 왜 `ret` 은 SET 에만 있나

타임라인이 부르는 정당한 질문: GET 은 복호 뒤(주차·재개)가 밖인데 SET 은
CQE 뒤(해제까지)가 `ret` 으로 안이다 — 통일해야 하지 않나?

주차 자체는 두 경로가 같다(완료 콜백은 드레인 안이라 연결을 재개할 수 없다,
§3-4). **차이는 효과 실현이 주차의 어느 쪽에 있는가다:**

```text
GET   CQE → [복호 = 실현, v3 끝] → 주차 → 재개 (unsuspend 뿐)
SET   CQE → 주차 → [trylock → WFLIGHT 해제 = 실현, v3 끝] → unsuspend
```

읽기의 효과(값 만들기)는 **사적 버퍼**라 락 없이 콜백 안에서 끝난다.
쓰기의 효과(공유 아이템 정규화)는 **`item_lock` 이 필요**한데, CQE 를
관측하는 문맥은 그 락을 이미 쥐고 있을 수 있다(in-place flush 는
`do_store_item` 의 락 아래, meta-get 도 — §3-2). 그래서 실현을 trylock
프로브 뒤에 뒀다 — **대기가 아니다.** 관측과 실현은 같은 호출에서 붙어
돌고(reap 틱: drain 2 줄 뒤 `flush_returns`), 다음 지점으로 미뤄지는 것은
락 아래 관측·trylock 충돌(~1/2^lock_power)·현재 연결 마지막 재개 셋뿐이다.
`ret` 실측 0.10~0.62 µs 가 그 "사실상 즉시" 의 값이다. EXP-0 의
173~2372 µs 는 이 구조가 아니라 방출구가 `if (out)` 에 막혀 있던
것이다(§2-4).
...
| 방향 | 결과 |
|---|---|
| SET `ret` 제거 (끝=CQE) | `WFLIGHT` 가 선 비정상 상태에서 측정 종료. **2371.84 µs 가 도로 계측 밖으로** — 기각 |
| GET 을 재개까지 연장 | 원격과 무관한 응답 스케줄링 포함 → "접근 지연" 이 아닌 다른 지표(이미 `bk`/`post` 가 잰다). 정의 변경만으로 pipe=1 GET 41.5 µs → 30 µs 계약 붕괴 |

**판단: 현행 유지.** 계약 정의의 변경은 관리자 결정 사항이다.
...
**요청이 기다릴 수 있는 곳이 두 군데 생긴다: 제출 전과 재개 전(SET 은 해제
전).** v3 의 두 문제는 정확히 이 두 자리에서 났다.
...
> ### 2026-08-06 실측 — **범인은 "20"이 아니라 io_queue 경로다**
>
> 위 서술은 "20 이 발화 불능이라 admit 이 폭증한다"로 읽힌다. **그 인과는
> 틀렸다.** 같은 빌드에서 inline 을 끄고 임계값만 바꿔 재면:
>
> ```text
> inline off, batch=3    GET 10.800 M   admit 67.95    (4 연결에서 발화 가능한 유일한 값)
> inline off, batch=20   GET 11.075 M   admit 67.76    발화 불능
> 차이                                   0.28%
> inline on  (운영점)     GET 13.037 M   admit  4.47    15 배 낮다
> ```
>
> **임계값을 발화 가능한 값으로 낮춰도 admit 이 안 변한다.** 이유는 산수에
> 있다 — 스레드당 연결이 4 개뿐이라 `batch=3` 은 **그 pass 의 마지막
> 연결에서만** 발화한다. 즉 발화하더라도 그 시점이 이미 pass 끝이다.
> 연결이 적으면 어떤 임계값도 pass 중간에 제출을 만들지 못한다.
>
> 따라서 원인은 **"제출을 pass 경계로 미루는 io_queue 경로 자체"** 이고,
> `ext_submit_batch` 는 그 경로 안의 손잡이일 뿐이다. `ext_submit_inline`
> 이 효과가 있는 이유도 임계값을 바꿔서가 아니라 **그 경로를 안 쓰기**
> 때문이다(§3-1 이 이미 그렇게 적고 있다 — 이제 인과까지 맞는다).
>
> 부수적으로 `EXP1_FINDINGS` 의 `submit_batch=4` 시험이 "무관"이라고 낸
> 결론은 **결과는 맞고 근거가 달라진다**: 발화가 0 회여서가 아니라, 발화해도
> pass 끝이기 때문이다.
>
> 근거 셀 `V-A-B3` / `V-A-B20` (`experiments/night-20260806/rows.tsv`).
...
자리에서 post 한다.** §1-1 v3 그림의 ③④(io_queue 삽입, pass 대기)가
경로에서 사라진다.
...
§1 의 재개 → 송신 구간은 v3 정의로도 **여전히 계측 밖**이다.
...
└ 서버 체류                      491 µs   21.6%
   ...
   └ post (복호→송신)                 360 µs   15.8%   ← span v3 의 16 배
```
