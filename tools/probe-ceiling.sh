...
# ═══════════════ 무장 전용 탐침: 총 in-flight 천장 ═══════════════
# 부하 없음. 무장만 시도해 어디서 bounce 등록이 깨지는지 이분한다.
# 알려진 것: 7,680 성공 · 30,720 실패. 그 사이를 찾는다.
export BIN='$HOME/coherent-mr-v2/bin/memcached.wfix'
say "천장 탐침 대기 — 앞 구동기 종료까지"
while pgrep -f 'bash /tmp/r4.sh' >/dev/null || pgrep -f 'bash /tmp/r4s.sh' >/dev/null; do sleep 120; done
say "천장 탐침 시작"
for q in 20 24 32; do
  WIRE=$((q*16)); SS=$((WIRE*2)); INF=$((30*WIRE))
  MB=$(python3 -c "print(f'{30*$SS*256/1048576:.2f}')")
  if SLOT=256 INLINE=1 AD=0 SS=$SS RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=$q ORD=0 \
       MCT=30 CPUSET=0-29 arm "PROBE-w$WIRE" >/dev/null 2>&1; then
    say "탐침 wire=$WIRE in-flight=$INF bounce=${MB}MB → 무장 성공"
  else
    say "탐침 wire=$WIRE in-flight=$INF bounce=${MB}MB → 무장 실패"
  fi
done
say "탐침 종료 — 서버를 운영점으로 되돌린다"
SLOT=256 INLINE=1 AD=0 SS=128 RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=4 ORD=0 MCT=30 CPUSET=0-29 \
  arm PROBE-restore-OP >/dev/null 2>&1 && say "운영점 복귀 완료" || say "운영점 복귀 실패 — 확인 필요"
