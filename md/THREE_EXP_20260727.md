...
측정일: 2026-07-27 (guest 재부팅 후 단일 실행 33 points + window=256 추가 실행 5 points)
...
| QP×depth | remote GET M/s | avg µs | p50 µs | p99 µs | (vs 128) M/s | (vs 128) avg µs |
|---:|---:|---:|---:|---:|---:|---:|
| 1×256 | 0.748 | 65.929 | 64.500 | 113.800 | ±0% | +0.3% |
| 2×128 | 1.557 | 63.684 | 61.900 | 115.200 | +2.2% | -2.2% |
| 4×64 | 2.627 | 61.687 | 63.100 | 115.200 | +4.3% | +67.4% |
| 8×32 | 2.733 | 33.745 | 32.100 | 112.600 | +2.1% | +60.0% |
| 16×16 | 2.712 | 20.784 | 19.200 | 55.000 | +2.4% | +49.0% |
...
- QP=1–2에서는 두 window의 결과가 사실상 같다. 실측 in-flight
  ...
  구간이기 때문이다.
- window가 실제로 바인딩되는 QP≥4에서는 256이 throughput을 +2–4%만 올리는
  대신 avg를 49–67% 올리고 p99를 최대 1.8배(8×32: 62.7→112.6µs) 악화시킨다.
  깊이를 두 배로 늘려 산 것은 대기시간뿐이다.
- pipeline=8의 throughput 천장(~2.7M/s)은 window 총량 제약이 아니다 —
  128→256으로 상한이 사실상 움직이지 않았다.
- 두 window 모두에서 같은 결론이 재현된다: **총량이 얼마든, 넓고 얕게 편
  분배가 좁고 깊은 분배를 지배한다.**
