...
# ═══════════════ 라운드 4 후속: S64x4 ═══════════════
# 캠페인에서 S64x4 는 create_cq(32,768 항목)로 죽었다 — bounce 까지 못 갔다.
# 그 CQ 는 이제 2×W+8 = 520 이고 bounce 는 3.75 MB 로 S32x8(성공)과 같다.
# 이 셀이 들어오면 형태 판정이 2점 쌍에서 3점 단조 추세 검정이 된다.
export BIN='$HOME/coherent-mr-v2/bin/memcached.wfix'
say "S64x4 대기 — 라운드 4 종료까지"
while pgrep -f 'bash /tmp/r4.sh' >/dev/null; do sleep 120; done
say "S64x4 시작"
SLOT=256 INLINE=1 AD=0 SS=512 RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=64 ORD=4 MCT=30 CPUSET=0-29 \
  arm R4-S64x4 \
  && go_trio R4-S64x4 30 4 256 64 \
     "port_v4 c33d13f9 slot=256 nqp=64 ORD=4 W=파생256 slots=512 — 형태 삼중점의 세 번째" \
  || say "R4-S64x4 무장 실패 (bounce 3.75MB 벽이면 이게 진짜 한계다)"
sfsave
say "S64x4 종료"
