...
```text
span v2   장치가 실제로 일한 시간          "얼마나 빨리 전송했나"
span v3   맡긴 순간 → 쓸 수 있는 순간      "얼마나 빨리 답했나"

GET   차이 = admit               (v2 는 post 부터, v3 는 진입부터. 끝은 같다)
SET   차이 = admit + ret         (v2 는 seal→CQE, v3 는 진입→WFLIGHT 해제)
```
