# semi_final 실험 명세 — 라운드 6 (수정 빌드 · uniform/zipfian 두 벌)
...
시간    부하 **60초**, 사이 20초.
        구성마다 **키 분포 2종 × YCSB 워크로드 3종 = 6부하**
        (관리자 결정 2026-08-07: 실험 1~8 전부 두 벌로 잰다)

          분포  U  --key-pattern=R:R                       uniform
                Z  --key-pattern=Z:Z --key-zipf-exp=0.99
                                     --key-zipf-scramble   YCSB Zipfian

          워크로드  -C  --ratio=0:1    read 100%
                    -B  --ratio=1:19   read 95% / update 5%
                    -A  --ratio=1:1    read 50% / update 50%

        순서는 U-C → U-B → U-A → Z-C → Z-B → Z-A 고정.
        genie 의 `-Z-A` 보고를 확인한 뒤 다음 구성으로 넘어간다.
        셀 이름은 `R6-<구성>-<U|Z>-<C|B|A>`.

        ★ 분포는 클라 전용이라 두 벌 사이에 **재기동도 재프리로드도 없다.**
          한 번 무장하고 6부하를 연달아 쏘므로 짝이 같은 bed·같은 무장에서
          나온다 — 라운드를 둘로 나누는 것보다 짝이 깨끗하고 2.7시간 짧다.
...
| 9 | 키 분포 (전용 축) | **uniform**, zipfian | 2 | 클라 `--key-pattern` 만 (서버 불변) |
...
11 R6-U · R6-Z                                    2×6   9번 축 (전용 대조)
```

각 줄의 `×N` 은 구성 수다. **구성마다 6부하**(U-C/B/A → Z-C/B/A)를 쏜다.
...
```text
구성  OP 2 + P 8 + E 8 + D 9 + C 16 + Q 6 + O 5 + S 5 + T 9 + 분포 2 = 70
부하  70 × 6 = 420.  구성당 6×60초 + 사이 20초×5 = 460초    ≈ 537분
...
1~8번 축이 이미 전 셀을 두 벌로 재므로 분포 효과는 격자 어디서나 읽힌다.
9번 축은 그와 별개로 **운영점에서의 분포 대조 하나를 독립 축으로 남긴다** —
이 캠페인이 답하려는 질문 중 하나가 "skew 가 이 설계에 무엇을 하는가" 이고,
그 답은 다른 축의 부산물이 아니라 자기 표를 가져야 한다.

```text
R6-U   --key-pattern=R:R                                     uniform
R6-Z   --key-pattern=Z:Z --key-zipf-exp=0.99 --key-zipf-scramble
각각 YCSB C → B → A, 60초. 서버 무변, 연달아 측정
```

`R6-OP` 의 U/Z 쌍과 구성이 같지만 따로 잰다. 가드 셀은 캠페인 시작·끝의
드리프트를 재는 것이 목적이라 그 사이에 11시간이 끼어 있고, 9번 축은 **연달아
찍은 짝**이어야 분포 효과만 남는다. 두 쌍이 어긋나면 그 차이가 곧 드리프트다.

### memtier zipf 와 YCSB zipf 의 차이
