...
enum store_item_type {
    ...
    STORE_PENDING /* v3 pac: 게시 완료, 응답은 WRITE CQE 이후 (logger status_map과 보조) */
};
...
/* v3 pac: 원격 WRITE가 wire에 있는 stub. 이 비트가 서 있는 동안
 * storage_delete는 loc을 회수하면 안 된다 — 회수하면 그 슬롯을 재할당받은
 * 다른 SET의 WRITE와 우리 WRITE가 같은 원격 메모리에 겹친다. 회수 권한은
 * 완료 콜백(storage_set_return_cb)이 갖는다. */
#define ITEM_WFLIGHT 8192
...
typedef struct {
    ...
    struct conn *cur_conn;      /* v3 pac: do_store_item이 suspend 대상 conn/resp를
                                 * 알아야 하는데 시그니처에 없다. 호출 직전 set,
                                 * 직후 clear — cur_async_ok가 명시 게이트다 */
    struct _mc_resp *cur_resp;
    bool cur_async_ok;          /* 이 호출자가 STORE_PENDING을 처리할 수 있다 */
