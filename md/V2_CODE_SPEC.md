...
전 point correctness 0.

**`ext_drain_spin` 튜닝 (같은 조건 W=16, mcT=14, p48)**

| spin | M/s | avg µs | p99 µs |
|---:|---:|---:|---:|
| 8 | 4.086 | 35.4 | 325.6 |
| 64 | 4.080 | 35.5 | 332.2 |
| 256 | 4.162 | 19.1 | 117.2 |
| 1024 | **4.208** | **15.3** | **33.4** |
| 4096 | 4.201 | 15.5 | 33.8 |

spin이 부족하면 완료가 이벤트 루프 회전까지 밀려 tail이 무너진다. 1024에서
포화하며 4096은 이득이 없다. **기본값을 1024로 확정한다.**

**shape 탐색 (W=16, spin=1024)**

| mcT | pipeline | M/s | avg µs | p99 µs |
|---:|---:|---:|---:|---:|
| 14 | 48 | 4.208 | 15.3 | 33.4 |
| 14 | 64 | 4.444 | 16.0 | 33.9 |
| 10 | 48 | 4.795 | 13.8 | 30.8 |
| 12 | 48 | 5.268 | 14.6 | 32.4 |
| 12 | **64** | **5.708** | **14.0** | **31.4** |
| 12 | 96 | 5.552 | 15.5 | 33.3 |

mcT=14는 worker14+IO2가 server 16코어를 꽉 채워 손해다. **v2 운영점은
mcT=12, W=16, pipeline=64, spin=1024**.

### P2c 게이트 결과 (2026-07-28) — 통과

동일 boot·동일 방법론(`/proc` utime+stime ÷ 완료 GET, v1 CPU_COST_ACCOUNTING과
같은 절차)으로 각자의 최적점에서 측정했다.

| | v1 (mcT8, QP8, d16, p48) | v2 (mcT12, W16, p64, spin1024) | 변화 |
|---|---:|---:|---:|
...
v1의 3.450µs/op는 `md/CPU_COST_ACCOUNTING.md`의 3.45와 일치해 측정 방법론이
교차 검증됐다. **P2c 게이트(≥5.5M/s AND CPU ≤2.2µs/op)를 둘 다 통과**했고,
§0에서 10M의 필요조건으로 유도한 **per-op CPU ≤2.0µs에 사실상 도달**했다
(2.021). 10M 환산 소요 코어 = 10M × 2.021µs ≈ 20.2 core-s/s — off-box
client로 guest 24 vCPU를 전부 서버에 주면 예산 안에 들어온다.
