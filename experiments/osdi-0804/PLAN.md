...
램프 편향  30초 창은 램프(정상 환산 손실 0.50초분) 탓에 **−1.66% 계통오차**
          (genie 실측, C1-64-GET 초당값). 라운드 내 셀끼리는 공통이라 델타에서
          소거. **교차비교만 보정**: 60초 기측정 대비 30초 쪽 +0.83%p,
          120초 대비 +1.24%p. 첫 30초 셀의 초당 추이로 0.50초분을 갱신하고
          SET 셀은 별도 확인 (WFLIGHT 램프가 더 길 수 있다)
