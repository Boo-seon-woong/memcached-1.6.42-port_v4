...
# ═══════════════ 곱 256 형태 스윕의 나머지 두 점 (무장 전용) ═══════════════
# 원안은 ORD 를 올려 형태를 바꿨지만 그 방향은 하드웨어가 막는다(ORD<=16).
# 같은 곱을 nqp 를 올려 만들면 형태가 5개가 된다 — 128x2, 256x1 이 되는지만 본다.
# 발자국은 다섯 점 모두 동일(slots 512, 워커당 128 KB)이라 순수 형태 대조가 된다.
export BIN='$HOME/coherent-mr-v2/bin/memcached.permr'
say "고-nqp 형태 탐침 대기"
while pgrep -f 'bash /tmp/r4.sh' >/dev/null || pgrep -f 'bash /tmp/r4s.sh' >/dev/null \
   || pgrep -f 'bash /tmp/prb.sh' >/dev/null || pgrep -f 'bash /tmp/pmv.sh' >/dev/null; do sleep 120; done
say "고-nqp 형태 탐침 시작"
for spec in 128:2 256:1; do
  q=${spec%%:*}; o=${spec##*:}
  if SLOT=256 INLINE=1 AD=0 SS=512 RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=$q ORD=$o \
       MCT=30 CPUSET=0-29 arm "SHAPE-${q}x${o}" >/dev/null 2>&1; then
    say "${q}x${o} (총 QP $((q*30))) → 무장 성공"
  else
    say "${q}x${o} (총 QP $((q*30))) → 무장 실패"
  fi
done
say "탐침 종료 — 운영점 복귀"
SLOT=256 INLINE=1 AD=0 SS=128 RE=8 PC=8 SQ=1 DEM=0 DVAL=64 NQP=4 ORD=0 MCT=30 CPUSET=0-29 \
  arm SHAPE-restore-OP >/dev/null 2>&1 && say "운영점 복귀 완료" || say "운영점 복귀 실패"
