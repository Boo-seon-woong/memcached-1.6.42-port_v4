...
**단일 요청은 v4 에서도 즉시 안 나간다.** `g_chain_n = 1 < 8` 이라 체인에
얹히기만 하고 `thread.c:522` 의 pass 끝 flush 에서 나간다 — stock 의
`thread_io_queue_submit()` 과 같은 지점이다. v4 가 바꾼 것은 **문턱**이고
(연결 20 → 요청 8), 고립된 한 건의 지연은 v3 와 같다.
